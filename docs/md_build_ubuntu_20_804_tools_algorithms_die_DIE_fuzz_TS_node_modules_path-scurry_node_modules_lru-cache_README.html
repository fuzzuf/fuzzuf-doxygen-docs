...
</div><!-- fragment --><h1><a class="anchor" id="autotoc_md2290"></a>
Usage</h1>
...
&lt;tt&gt;class LRUCache&lt;K, V, FC = unknown&gt;(options)&lt;/tt&gt;</h1>
<p>Create a new <code>LRUCache</code> object.</p>
<p>When using TypeScript, set the <code>K</code> and <code>V</code> types to the <code>key</code> and <code>value</code> types, respectively.</p>
<p>The <code>FC</code> ("fetch context") generic type defaults to <code>unknown</code>. If set to a value other than <code>void</code> or <code>undefined</code>, then any calls to <code>cache.fetch()</code> <em>must</em> provide a <code>context</code> option matching the <code>FC</code> type. If <code>FC</code> is set to <code>void</code> or <code>undefined</code>, then <code>cache.fetch()</code> <em>must not</em> provide a <code>context</code> option. See the documentation on <code>async fetch()</code> below.</p>
<h1><a class="anchor" id="autotoc_md2292"></a>
Options</h1>
<p>All options are available on the LRUCache instance, making it safe to pass an LRUCache instance as the options argument to make another empty cache of the same type.</p>
<p>Some options are marked read-only because changing them after instantiation is not safe. Changing any of the other options will of course only have an effect on subsequent method calls.</p>
<h2><a class="anchor" id="autotoc_md2293"></a>
&lt;tt&gt;max&lt;/tt&gt; (read only)</h2>
...
&lt;tt&gt;fetchMethod&lt;/tt&gt; (read only)</h2>
...
<p>Unless <code>ignoreFetchAbort</code> is also set, the underlying <code>fetchMethod</code> will still be considered canceled, and any value it returns will be ignored and not cached.</p>
<p>Caveat: since fetches are aborted when a new value is explicitly set in the cache, this can lead to fetch returning a stale value, since that was the fallback value <em>at the moment the <code>fetch()</code> was initiated</em>, even though the new updated value is now present in the cache.</p>
<p>For example:</p>
<div class="fragment"><div class="line">const cache = new LRUCache&lt;string, any&gt;({</div>
<div class="line">  ttl: 100,</div>
<div class="line">  fetchMethod: async (url, oldValue, { signal }) =&gt; {</div>
<div class="line">    const res = await fetch(url, { signal })</div>
<div class="line">    return await res.json()</div>
<div class="line">  },</div>
<div class="line">})</div>
<div class="line">cache.set(&#39;https://example.com/&#39;, { some: &#39;data&#39; })</div>
<div class="line">// 100ms go by...</div>
<div class="line">const result = cache.fetch(&#39;https://example.com/&#39;)</div>
<div class="line">cache.set(&#39;https://example.com/&#39;, { other: &#39;thing&#39; })</div>
<div class="line">console.log(await result) // { some: &#39;data&#39; }</div>
<div class="line">console.log(cache.get(&#39;https://example.com/&#39;)) // { other: &#39;thing&#39; }</div>
...
<p>Unlike several other options, this may <em>not</em> be overridden by passing an option to <code>set()</code>, for performance reasons.</p>
...
<li><code>delete</code> Item was removed by explicit <code>cache.delete(key)</code> or by calling <code><a class="el" href="range__traits_8hpp.html#a29625773406e727f503a10af9fa8ea0c">cache.clear()</a></code>, which deletes everything.</li>
</ul>
...
&lt;tt&gt;disposeAfter&lt;/tt&gt; (read only)</h2>
...
<p>If ttl tracking is enabled, and <code>max</code> and <code>maxSize</code> are not set, and <code>ttlAutopurge</code> is not set, then a warning will be emitted cautioning about the potential for unbounded memory consumption. (The TypeScript definitions will also discourage this.)</p>
...
&lt;tt&gt;new LRUCache&lt;K, V, FC = unknown&gt;(options)&lt;/tt&gt;</h2>
<p>Create a new LRUCache. All options are documented above, and are on the cache as public members.</p>
<p>The <code>K</code> and <code>V</code> types define the key and value types, respectively. The optional <code>FC</code> type defines the type of the <code>context</code> object passed to <code>cache.fetch()</code>.</p>
<p>Keys and values <b>must not</b> be <code>null</code> or <code>undefined</code>.</p>
...
<p>If the value is <code>undefined</code>, then this is an alias for <code>cache.delete(key)</code>. <code>undefined</code> is never stored in the cache. See <b>Storing Undefined Values</b> below.</p>
...
&lt;tt&gt;info(key) =&gt; Entry | undefined&lt;/tt&gt;</h2>
<p>Return an <code>Entry</code> object containing the currently cached value, as well as ttl and size information if available. Returns undefined if the key is not found in the cache.</p>
<p>Unlike <code>dump()</code> (which is designed to be portable and survive serialization), the <code>start</code> value is always the current timestamp, and the <code>ttl</code> is a calculated remaining time to live (negative if expired).</p>
<p>Note that stale values are always returned, rather than being pruned and treated as if they were not in the cache. If you wish to exclude stale entries, guard against a negative <code>ttl</code> value.</p>
...
<li><code>context</code> - sets the <code>context</code> option passed to the underlying <code>fetchMethod</code>.</li>
</ul>
...
<h3><a class="anchor" id="autotoc_md2322"></a>
Setting &lt;tt&gt;context&lt;/tt&gt;</h3>
<p>If an <code>FC</code> type is set to a type other than <code>unknown</code>, <code>void</code>, or <code>undefined</code> in the LRUCache constructor, then all calls to <code>cache.fetch()</code> <em>must</em> provide a <code>context</code> option. If set to <code>undefined</code> or <code>void</code>, then calls to fetch <em>must not</em> provide a <code>context</code> option.</p>
<p>The <code>context</code> param allows you to provide arbitrary data that might be relevant in the course of fetching the data. It is only relevant for the course of a single <code>fetch()</code> operation, and discarded afterwards.</p>
<h3><a class="anchor" id="autotoc_md2323"></a>
Note: &lt;tt&gt;fetch()&lt;/tt&gt; calls are inflight-unique</h3>
<p>If you call <code>fetch()</code> multiple times with the same key value, then every call after the first will resolve on the same promise<sup>1</sup>, <em>even if they have different settings that would otherwise change the behvavior of the fetch</em>, such as <code>noDeleteOnFetchRejection</code> or <code>ignoreFetchAbort</code>.</p>
<p>In most cases, this is not a problem (in fact, only fetching something once is what you probably want, if you're caching in the first place). If you are changing the fetch() options dramatically between runs, there's a good chance that you might be trying to fit divergent semantics into a single object, and would be better off with multiple cache instances.</p>
<p><b>1</b>: Ie, they're not the "same Promise", but they resolve at the same time, because they're both waiting on the same underlying fetchMethod response.</p>
...
<h2><a class="anchor" id="autotoc_md2325"></a>
...
<p>For the usage of the <code>status</code> option, see <b>Status Tracking</b> below.</p>
<h2><a class="anchor" id="autotoc_md2326"></a>
&lt;tt&gt;delete(key)&lt;/tt&gt;</h2>
<p>Deletes a key out of the cache.</p>
<p>Returns <code>true</code> if the key was deleted, <code>false</code> otherwise.</p>
<h2><a class="anchor" id="autotoc_md2327"></a>
&lt;tt&gt;clear()&lt;/tt&gt;</h2>
<p><a class="el" href="classClear.html" title="call clear() member function of the value specified by the Path. The node takes 1 path for value to c...">Clear</a> the cache entirely, throwing away all values.</p>
...
<p>```ts interface Status&lt;V&gt; { /**</p><ul>
<li>The status of a set() operation.</li>
<li></li>
<li>- add: the item was not found in the cache, and was added</li>
<li>- update: the item was in the cache, with the same value provided</li>
<li>- replace: the item was in the cache, and replaced</li>
<li>- miss: the item was not added to the cache for some reason */ set?: 'add' | 'update' | 'replace' | 'miss'</li>
</ul>
<p>/**</p><ul>
<li>the ttl stored for the item, or undefined if ttls are not used. */ ttl?: LRUMilliseconds</li>
</ul>
<p>/**</p><ul>
<li>the start time for the item, or undefined if ttls are not used. */ start?: LRUMilliseconds</li>
</ul>
<p>/**</p><ul>
<li>The timestamp used for TTL calculation */ now?: LRUMilliseconds</li>
</ul>
<p>/**</p><ul>
<li>the remaining ttl for the item, or undefined if ttls are not used. */ remainingTTL?: LRUMilliseconds</li>
</ul>
<p>/**</p><ul>
<li>The calculated size for the item, if sizes are used. */ size?: LRUSize</li>
</ul>
<p>/**</p><ul>
<li>A flag indicating that the item was not stored, due to exceeding the</li>
<li><a class="el" href="">maxEntrySize</a> */ maxEntrySizeExceeded?: true</li>
</ul>
<p>/**</p><ul>
<li>The old value, specified in the case of &lsquo;set:'update&rsquo;<code>or *</code>set:'replace'` */ oldValue?: V</li>
</ul>
<p>/**</p><ul>
<li>The results of a <a class="el" href="">has</a> operation</li>
<li></li>
<li>- hit: the item was found in the cache</li>
<li>- stale: the item was found in the cache, but is stale</li>
<li>- miss: the item was not found in the cache */ has?: 'hit' | 'stale' | 'miss'</li>
</ul>
<p>/**</p><ul>
<li>The status of a <a class="el" href="">fetch</a> operation.</li>
<li>Note that this can change as the underlying fetch() moves through</li>
<li>various states.</li>
<li></li>
<li>- inflight: there is another fetch() for this key which is in process</li>
<li>- get: there is no fetchMethod, so <a class="el" href="">get</a> was called.</li>
<li>- miss: the item is not in cache, and will be fetched.</li>
<li>- hit: the item is in the cache, and was resolved immediately.</li>
<li>- stale: the item is in the cache, but stale.</li>
<li>- refresh: the item is in the cache, and not stale, but</li>
<li><a class="el" href="">forceRefresh</a> was specified. */ fetch?: 'get' | 'inflight' | 'miss' | 'hit' | 'stale' | 'refresh'</li>
</ul>
<p>/**</p><ul>
<li>The <a class="el" href="">fetchMethod</a> was called */ fetchDispatched?: true</li>
</ul>
<p>/**</p><ul>
<li>The cached value was updated after a successful call to fetchMethod */ fetchUpdated?: true</li>
</ul>
<p>/**</p><ul>
<li>The reason for a fetch() rejection. Either the error raised by the</li>
<li><a class="el" href="">fetchMethod</a>, or the reason for an AbortSignal. */ fetchError?: Error</li>
</ul>
<p>/**</p><ul>
<li>The fetch received an abort signal */ fetchAborted?: true</li>
</ul>
<p>/**</p><ul>
<li>The abort signal received was ignored, and the fetch was allowed to</li>
<li>continue. */ fetchAbortIgnored?: true</li>
</ul>
<p>/**</p><ul>
<li>The fetchMethod promise resolved successfully */ fetchResolved?: true</li>
</ul>
<p>/**</p><ul>
<li>The results of the fetchMethod promise were stored in the cache */ fetchUpdated?: true</li>
</ul>
<p>/**</p><ul>
<li>The fetchMethod promise was rejected */ fetchRejected?: true</li>
</ul>
<p>/**</p><ul>
<li>The status of a <a class="el" href="">get</a> operation.</li>
<li></li>
<li>- fetching: The item is currently being fetched. If a previous value is</li>
<li>present and allowed, that will be returned.</li>
<li>- stale: The item is in the cache, and is stale.</li>
<li>- hit: the item is in the cache</li>
<li>- miss: the item is not in the cache */ get?: 'stale' | 'hit' | 'miss'</li>
</ul>
<p>/**</p><ul>
<li>A fetch or get operation returned a stale value. */ returnedStale?: true } <div class="fragment"><div class="line">## Storage Bounds Safety</div>
<div class="line"> </div>
<div class="line">This implementation aims to be as flexible as possible, within</div>
<div class="line">the limits of safe memory consumption and optimal performance.</div>
<div class="line"> </div>
<div class="line">At initial object creation, storage is allocated for `max` items.</div>
<div class="line">If `max` is set to zero, then some performance is lost, and item</div>
<div class="line">count is unbounded. Either `maxSize` or `ttl` _must_ be set if</div>
<div class="line">`max` is not specified.</div>
<div class="line"> </div>
<div class="line">If `maxSize` is set, then this creates a safe limit on the</div>
<div class="line">maximum storage consumed, but without the performance benefits of</div>
<div class="line">pre-allocation. When `maxSize` is set, every item _must_ provide</div>
<div class="line">a size, either via the `sizeCalculation` method provided to the</div>
<div class="line">constructor, or via a `size` or `sizeCalculation` option provided</div>
<div class="line">to `cache.set()`. The size of every item _must_ be a positive</div>
<div class="line">integer.</div>
<div class="line"> </div>
<div class="line">If neither `max` nor `maxSize` are set, then `ttl` tracking must</div>
<div class="line">be enabled. Note that, even when tracking item `ttl`, items are</div>
<div class="line">_not_ preemptively deleted when they become stale, unless</div>
<div class="line">`ttlAutopurge` is enabled. Instead, they are only purged the</div>
<div class="line">next time the key is requested. Thus, if `ttlAutopurge`, `max`,</div>
<div class="line">and `maxSize` are all not set, then the cache will potentially</div>
<div class="line">grow unbounded.</div>
<div class="line"> </div>
<div class="line">In this case, a warning is printed to standard error. Future</div>
<div class="line">versions may require the use of `ttlAutopurge` if `max` and</div>
<div class="line">`maxSize` are not specified.</div>
<div class="line"> </div>
<div class="line">If you truly wish to use a cache that is bound _only_ by TTL</div>
<div class="line">expiration, consider using a `Map` object, and calling</div>
<div class="line">`setTimeout` to delete entries when they expire. It will perform</div>
<div class="line">much better than an LRU cache.</div>
<div class="line"> </div>
<div class="line">Here is an implementation you may use, under the same</div>
<div class="line">[license](./LICENSE) as this package:</div>
<div class="line"> </div>
<div class="line">```js</div>
<div class="line">// a storage-unbounded ttl cache that is not an lru-cache</div>
...
<p>If that isn't to your liking, check out <a href="http://npm.im/@isaacs/ttlcache">@isaacs/ttlcache</a>.</p>
<h1><a class="anchor" id="autotoc_md2343"></a>
Storing Undefined Values</h1>
<p>This cache never stores undefined values, as <code>undefined</code> is used internally in a few places to indicate that a key is not in the cache.</p>
<p>You may call <code>cache.set(key, undefined)</code>, but this is just an an alias for <code>cache.delete(key)</code>. Note that this has the effect that <code>cache.has(key)</code> will return <em>false</em> after setting it to undefined.</p>
<div class="fragment"><div class="line">cache.set(myKey, undefined)</div>
<div class="line">cache.has(myKey) // false!</div>
</div><!-- fragment --><p>If you need to track <code>undefined</code> values, and still note that the key is in the cache, an easy workaround is to use a sigil object of your own.</p>
...
<div class="line">}</div>
</div><!-- fragment --><h1><a class="anchor" id="autotoc_md2344"></a>
Performance</h1>
...
<ul>
<li>The <code>fetchContext</code> option was renamed to <code>context</code>, and may no longer be set on the cache instance itself.</li>
