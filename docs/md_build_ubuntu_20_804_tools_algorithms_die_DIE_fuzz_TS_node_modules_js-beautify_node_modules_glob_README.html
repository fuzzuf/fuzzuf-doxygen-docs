...
<div class="contents">
<div class="textblock"><p>Match files using the patterns the shell uses.</p>
...
<p>```js // load using import import { glob, globSync, globStream, globStreamSync, Glob } from 'glob' // or using commonjs, that's fine, too const { glob, globSync, globStream, globStreamSync, Glob, } = require('glob')</p>
<p>// the main glob() and globSync() resolve/return array of filenames</p>
<p>// all js files, but don't look in node_modules const jsfiles = await glob('**/*.js', { ignore: 'node_modules/**' })</p>
<p>// pass in a signal to cancel the glob walk const stopAfter100ms = await glob('**/*.css', { signal: AbortSignal.timeout(100), })</p>
<p>// multiple patterns supported as well const images = await glob(['css/*.{png,jpeg}', 'public/*.{png,jpeg}'])</p>
<p>// but of course you can do that with the glob pattern also // the sync function is the same, just returns a string[] instead // of Promise&lt;string[]&gt; const imagesAlt = globSync('{css,public}/*.{png,jpeg}')</p>
<p>// you can also stream them, this is a Minipass stream const filesStream = globStream(['**/*.dat', 'logs/**/*.log'])</p>
<p>// construct a Glob object if you wanna do it that way, which // allows for much faster walks if you have to look in the same // folder multiple times. const g = new Glob('**/foo', {}) // glob objects are async iterators, can also do globIterate() or // g.iterate(), same deal for await (const file of g) { console.log('found a foo <a href="file:',">file:',</a> file) } // pass a glob as the glob options to reuse its settings and caches const g2 = new Glob('**/bar', g) // sync iteration works as well for (const file of g2) { console.log('found a bar <a href="file:',">file:',</a> file) }</p>
<p>// you can also pass withFileTypes: true to get Path objects // these are like a Dirent, but with some more added powers // check out <a href="http://npm.im/path-scurry">http://npm.im/path-scurry</a> for more info on their API const g3 = new Glob('**/baz/**', { withFileTypes: true }) g3.stream().on('data', path =&gt; { console.log( 'got a path object', path.fullpath(), path.isDirectory(), path.readdirSync().map(e =&gt; e.name) ) })</p>
<p>// if you use stat:true and withFileTypes, you can sort results // by things like modified time, filter by permission mode, etc. // All Stats fields will be available in that case. Slightly // slower, though. // For example: const results = await glob('**', { stat: true, withFileTypes: true })</p>
<p>const timeSortedFiles = results .sort((a, b) =&gt; a.mtimeMs - b.mtimeMs) .map(path =&gt; path.fullpath())</p>
<p>const groupReadableFiles = results .filter(path =&gt; path.mode &amp; 0o040) .map(path =&gt; path.fullpath())</p>
<p>// custom ignores can be done like this, for example by saying // you'll ignore all markdown files, and all folders named 'docs' const customIgnoreResults = await glob('**', { ignore: { ignored: p =&gt; /.md$/.test(p.name), childrenIgnored: p =&gt; p.isNamed('docs'), }, })</p>
<p>// another fun use case, only return files with the same name as // their parent folder, plus either <code>.ts</code> or <code>.js</code> const folderNamedModules = await glob('**/*.{ts,js}', { ignore: { ignored: p =&gt; { const pp = p.parent return !(p.isNamed(pp.name + '.ts') || p.isNamed(pp.name + '.js')) }, }, })</p>
<p>// find all files edited in the last hour, to do this, we ignore // all of them that are more than an hour old const newFiles = await glob('**', { // need stat so we have mtime stat: true, // only want the files, not the dirs nodir: true, ignore: { ignored: p =&gt; { return new Date() - p.mtime &gt; 60 * 60 * 1000 }, // could add similar childrenIgnored here as well, but // directory mtime is inconsistent across platforms, so // probably better not to, unless you know the system // tracks this reliably. }, }) </p><div class="fragment"><div class="line">**Note** Glob patterns should always use `/` as a path separator,</div>
<div class="line">even on Windows systems, as `\` is used to escape glob</div>
<div class="line">characters. If you wish to use `\` as a path separator _instead</div>
<div class="line">of_ using it as an escape character on Windows platforms, you may</div>
<div class="line">set `windowsPathsNoEscape:true` in the options. In this mode,</div>
<div class="line">special glob characters cannot be escaped, making it impossible</div>
<div class="line">to match a literal `*` `?` and so on in filenames.</div>
<div class="line"> </div>
<div class="line">## Command Line Interface</div>
</div><!-- fragment --><p>$ glob -h</p>
<p>Usage: glob [options] [&lt;pattern&gt; [&lt;pattern&gt; ...]]</p>
<p>Expand the positional glob expression arguments into any matching file system paths found.</p>
<p>-c&lt;command&gt; &ndash;cmd=&lt;command&gt; Run the command provided, passing the glob expression matches as arguments.</p>
<p>-A &ndash;all By default, the glob cli command will not expand any arguments that are an exact match to a file on disk.</p>
<p>This prevents double-expanding, in case the shell expands an argument whose filename is a glob expression.</p>
<p>For example, if 'app/*.ts' would match 'app/[id].ts', then on Windows powershell or cmd.exe, 'glob app/*.ts' will expand to 'app/[id].ts', as expected. However, in posix shells such as bash or zsh, the shell will first expand 'app/*.ts' to a list of filenames. Then glob will look for a file matching 'app/[id].ts' (ie, 'app/i.ts' or 'app/d.ts'), which is unexpected.</p>
<p>Setting '&ndash;all' prevents this behavior, causing glob to treat ALL patterns as glob expressions to be expanded, even if they are an exact match to a file on disk.</p>
<p>When setting this option, be sure to enquote arguments so that the shell will not expand them prior to passing them to the glob command process.</p>
<p>-a &ndash;absolute Expand to absolute paths -d &ndash;dot-relative Prepend './' on relative matches -m &ndash;mark Append a / on any directories matched -x &ndash;posix Always resolve to posix style paths, using '/' as the directory separator, even on Windows. Drive letter absolute matches on Windows will be expanded to their full resolved UNC maths, eg instead of 'C:\foo\bar', it will expand to '//?/C:/foo/bar'.</p>
<p>-f &ndash;follow Follow symlinked directories when expanding '**' -R &ndash;realpath Call 'fs.realpath' on all of the results. In the case of an entry that cannot be resolved, the entry is omitted. This incurs a slight performance penalty, of course, because of the added system calls.</p>
<p>-s &ndash;stat Call 'fs.lstat' on all entries, whether required or not to determine if it's a valid match.</p>
<p>-b &ndash;match-base Perform a basename-only match if the pattern does not contain any slash characters. That is, '*.js' would be treated as equivalent to '**/*.js', matching js files in all directories.</p>
<p>&ndash;dot Allow patterns to match files/directories that start with '.', even if the pattern does not start with '.'</p>
<p>&ndash;nobrace Do not expand {...} patterns &ndash;nocase Perform a case-insensitive match. This defaults to 'true' on macOS and Windows platforms, and false on all others.</p>
<p>Note: 'nocase' should only be explicitly set when it is known that the filesystem's case sensitivity differs from the platform default. If set 'true' on case-insensitive file systems, then the walk may return more or less results than expected.</p>
<p>&ndash;nodir Do not match directories, only files. </p><pre class="fragment">                   Note: to *only* match directories, append a '/' at the
                   end of the pattern.
</pre><p>&ndash;noext Do not expand extglob patterns, such as '+(a|b)' &ndash;noglobstar Do not expand '**' against multiple path portions. Ie, treat it as a normal '*' instead.</p>
<p>&ndash;windows-path-no-escape Use '\' as a path separator <em>only</em>, and <em>never</em> as an escape character. If set, all '\' characters are replaced with '/' in the pattern.</p>
<p>-D&lt;n&gt; &ndash;max-depth=&lt;n&gt; Maximum depth to traverse from the current working directory</p>
<p>-C&lt;cwd&gt; &ndash;cwd=&lt;cwd&gt; Current working directory to execute/match in -r&lt;root&gt; &ndash;root=&lt;root&gt; A string path resolved against the 'cwd', which is used as the starting point for absolute patterns that start with '/' (but not drive letters or UNC paths on Windows).</p>
<p>Note that this <em>doesn't</em> necessarily limit the walk to the 'root' directory, and doesn't affect the cwd starting point for non-absolute patterns. A pattern containing '..' will still be able to traverse out of the root directory, if it is not an actual root directory on the filesystem, and any non-absolute patterns will still be matched in the 'cwd'.</p>
<p>To start absolute and non-absolute patterns in the same path, you can use '&ndash;root=' to set it to the empty string. However, be aware that on Windows systems, a pattern like 'x:/*' or '//host/share/*' will <em>always</em> start in the 'x:/' or '//host/share/' directory, regardless of the &ndash;root setting.</p>
<p>&ndash;platform=&lt;platform&gt; Defaults to the value of 'process.platform' if available, or 'linux' if not. Setting &ndash;platform=win32 on non-Windows systems may cause strange behavior!</p>
<p>-i&lt;ignore&gt; &ndash;ignore=&lt;ignore&gt; Glob patterns to ignore Can be set multiple times -v &ndash;debug Output a huge amount of noisy debug information about patterns as they are parsed and used to match files.</p>
<p>-h &ndash;help Show this usage information ```</p>
<h1><a class="anchor" id="autotoc_md1992"></a>
&lt;tt&gt;glob(pattern: string | string[], options?: GlobOptions) =&gt; Promise&lt;string[] | Path[]&gt;&lt;/tt&gt;</h1>
...
&lt;tt&gt;globSync(pattern: string | string[], options?: GlobOptions) =&gt; string[] | Path[]&lt;/tt&gt;</h1>
<p>Synchronous form of <code>glob()</code>.</p>
<p>Alias: <code>glob.sync()</code></p>
<h1><a class="anchor" id="autotoc_md1994"></a>
&lt;tt&gt;globIterate(pattern: string | string[], options?: GlobOptions) =&gt; AsyncGenerator&lt;string&gt;&lt;/tt&gt;</h1>
...
&lt;tt&gt;globIterateSync(pattern: string | string[], options?: GlobOptions) =&gt; Generator&lt;string&gt;&lt;/tt&gt;</h1>
...
&lt;tt&gt;globStream(pattern: string | string[], options?: GlobOptions) =&gt; Minipass&lt;string | Path&gt;&lt;/tt&gt;</h1>
...
&lt;tt&gt;globStreamSync(pattern: string | string[], options?: GlobOptions) =&gt; Minipass&lt;string | Path&gt;&lt;/tt&gt;</h1>
...
&lt;tt&gt;hasMagic(pattern: string | string[], options?: GlobOptions) =&gt; boolean&lt;/tt&gt;</h1>
<p>Returns <code>true</code> if the provided pattern contains any "magic" glob characters, given the options provided.</p>
<p>Brace expansion is not considered "magic" unless the <code>magicalBraces</code> option is set, as brace expansion just turns one string into an array of strings. So a pattern like &lsquo;'x{a,b}y&rsquo;<code> would return</code>false<code>, because</code>'xay'<code>and</code>'xby'` both do not contain any magic glob characters, and it's treated the same as if you had called it on &lsquo;['xay&rsquo;, 'xby']<code>. When </code>magicalBraces:true` is in the options, brace expansion <em>is</em> treated as a pattern having magic.</p>
<h1><a class="anchor" id="autotoc_md1999"></a>
&lt;tt&gt;escape(pattern: string, options?: GlobOptions) =&gt; string&lt;/tt&gt;</h1>
...
&lt;tt&gt;unescape(pattern: string, options?: GlobOptions) =&gt; string&lt;/tt&gt;</h1>
...
&lt;tt&gt;const g = new Glob(pattern: string | string[], options: GlobOptions)&lt;/tt&gt;</h2>
...
<li><p class="startli"><code>cwd</code> String path or <code><a href="file://">file://</a></code> string or URL object. The current working directory in which to search. Defaults to <code>process.cwd()</code>. See also: "Windows, CWDs, Drive Letters, and
  UNC Paths", below.</p>
<p class="startli">This option may be eiher a string path or a <code><a href="file://">file://</a></code> URL object or string.</p>
</li>
...
<li><p class="startli"><code>magicalBraces</code> Treat brace expansion like <code>{a,b}</code> as a "magic" pattern. Has no effect if <a class="el" href="">nobrace</a> is set.</p>
<p class="startli">Only has effect on the <a class="el" href="">hasMagic</a> function, no effect on glob pattern matching itself.</p>
</li>
...
<li><code>nodir</code> Do not match directories, only files. (Note: to match <em>only</em> directories, put a <code>/</code> at the end of the pattern.)</li>
...
</div><!-- fragment --><h1><a class="anchor" id="autotoc_md2015"></a>
...
<h3><a class="anchor" id="autotoc_md2021"></a>
Drive Letters</h3>
...
</div><!-- fragment --><h1><a class="anchor" id="autotoc_md2026"></a>
...
<p>In my testing, <code>fast-glob</code> is around 10-20% faster than this module when walking over 200k files nested 4 directories deep<a href="#fn-webscale">1</a>. However, there are some inconsistencies with Bash matching behavior that this module does not suffer from:</p>
<ul>
