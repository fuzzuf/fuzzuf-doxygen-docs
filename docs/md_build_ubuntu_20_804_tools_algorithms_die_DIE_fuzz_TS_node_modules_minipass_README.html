...
<p>Supports <code>pipe()</code>ing (including multi-<code>pipe()</code> and backpressure transmission), buffering data until either a <code>data</code> event handler or <code>pipe()</code> is added (so you don't lose the first chunk), and most other cases where PassThrough is a good idea.</p>
...
<p><code>objectMode</code> can only be set at instantiation. Attempting to write something other than a String or Buffer without having set <code>objectMode</code> in the options will throw an error.</p>
...
<ul>
<li><code>RType</code> the type being read, which defaults to <code>Buffer</code>. If <code>RType</code> is <code>string</code>, then the constructor <em>must</em> get an options object specifying either an <code>encoding</code> or <code>objectMode: true</code>. If it's anything other than <code>string</code> or <code>Buffer</code>, then it <em>must</em> get an options object specifying <code>objectMode: true</code>.</li>
<li><code>WType</code> the type being written. If <code>RType</code> is <code>Buffer</code> or <code>string</code>, then this defaults to <code>ContiguousData</code> (Buffer, string, ArrayBuffer, or ArrayBufferView). Otherwise, it defaults to <code>RType</code>.</li>
<li><code>Events</code> type mapping event names to the arguments emitted with that event, which extends <code>Minipass.Events</code>.</li>
</ul>
...
<div class="line">})</div>
</div><!-- fragment --><p>Emitting/handling events that aren't declared in this way is fine, but the arguments will be typed as <code>unknown</code>.</p>
...
<p>You can avoid most of these differences entirely (for a very small performance penalty) by setting <code>{async: true}</code> in the constructor options.</p>
...
<p>If you wish to have a Minipass stream with behavior that more closely mimics Node.js core streams, you can set the stream in async mode either by setting <code>async: true</code> in the constructor options, or by setting <code>stream.async = true</code> later on.</p>
<div class="fragment"><div class="line">// hybrid module, either works</div>
<div class="line">import { Minipass } from &#39;minipass&#39;</div>
<div class="line">// or:</div>
<div class="line">const { Minipass } = require(&#39;minipass&#39;)</div>
...
<div class="line">// hello</div>
<div class="line">// world</div>
</div><!-- fragment --><h2><a class="anchor" id="autotoc_md2161"></a>
...
<p>Ie, if you do &lsquo;stream.on('end&rsquo;, someFunction)<code>, and the stream has already emitted</code>end`, then it will call the handler right away. (You can think of this somewhat like attaching a new <code>.then(fn)</code> to a previously-resolved Promise.)</p>
...
Emit &lt;tt&gt;error&lt;/tt&gt; When Asked</h2>
<p>The most recent error object passed to the &lsquo;'error&rsquo;<code>event is stored on the stream. If a new</code>'error'` event handler is added, and an error was previously emitted, then the event handler will be called immediately (or on <code>process.nextTick</code> in the case of async streams).</p>
...
</div><!-- fragment --><p>All of the hazards in this section are avoided by setting <code>{ async: true }</code> in the Minipass constructor, or by setting <code>stream.async = true</code> afterwards. Note that this does add some overhead, so should only be done in cases where you are willing to lose a bit of performance in order to avoid having to refactor program logic.</p>
...
</div><!-- fragment --><h2><a class="anchor" id="autotoc_md2168"></a>
OPTIONS</h2>
<ul>
...
<li><code>async</code> Defaults to <code>false</code>. Set to <code>true</code> to defer data emission until next tick. This reduces performance slightly, but makes Minipass streams use timing behavior closer to Node core streams. See <a href="#timing">Timing</a> for more details.</li>
<li><code>signal</code> An <code>AbortSignal</code> that will cause the stream to unhook itself from everything and become as inert as possible. Note that providing a <code>signal</code> parameter will make &lsquo;'error&rsquo;` events no longer throw if they are unhandled, but they will still be emitted to handlers if any are attached.</li>
</ul>
...
<li><code>pipe(dest)</code> - Send all output to the stream provided. When data is emitted, it is immediately written to any and all pipe destinations. (Or written on next tick in <code>async</code> mode.)</li>
<li><code>unpipe(dest)</code> - Stop piping to the destination stream. This is immediate, meaning that any asynchronously queued data will <em>not</em> make it to the destination when running in <code>async</code> mode.<ul>
<li><code>options.end</code> - Boolean, end the destination stream when the source stream ends. Default <code>true</code>.</li>
<li><code>options.proxyErrors</code> - Boolean, proxy <code>error</code> events from the source stream to the destination stream. Note that errors are <em>not</em> proxied after the pipeline terminates, either due to the source emitting &lsquo;'end&rsquo;<code>or manually unpiping with </code>src.unpipe(dest)<code>. Default</code>false<code>. -</code>on(ev, fn)<code>,</code>emit(ev, fn)` - Minipass streams are EventEmitters. Some events are given special treatment, however. (See below under "events".)</li>
</ul>
</li>
...
<li><code>pipes</code> An array of Pipe objects referencing streams that this stream is piping into.</li>
...
<li><code>aborted</code> Readonly property set when the <code>AbortSignal</code> dispatches an <code>abort</code> event.</li>
</ul>
<h2><a class="anchor" id="autotoc_md2172"></a>
Events</h2>
<ul>
...
<div class="line">  }</div>
<div class="line">)</div>
</div><!-- fragment --><h2><a class="anchor" id="autotoc_md2176"></a>
collecting</h2>
...
<div class="line">})</div>
</div><!-- fragment --><h2><a class="anchor" id="autotoc_md2177"></a>
...
<div class="line">})</div>
</div><!-- fragment --><h2><a class="anchor" id="autotoc_md2178"></a>
iteration</h2>
...
<p>// some source of some data let i = 5 const inter = setInterval(() =&gt; { if (i&ndash; &gt; 0) mp.write(Buffer.from('foo<br  />
...
</div><!-- fragment --><h2><a class="anchor" id="autotoc_md2179"></a>
...
</div><!-- fragment --><h2><a class="anchor" id="autotoc_md2180"></a>
...
<div class="line">  }</div>
<div class="line">}</div>
</div><!-- fragment --><h2><a class="anchor" id="autotoc_md2181"></a>
...
<div class="line">  }</div>
<div class="line">}</div>
</div><!-- fragment --><h2><a class="anchor" id="autotoc_md2182"></a>
