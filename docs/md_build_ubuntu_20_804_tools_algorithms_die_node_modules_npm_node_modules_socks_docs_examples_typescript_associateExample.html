...
<div class="contents">
<div class="textblock"><h1><a class="anchor" id="autotoc_md10395"></a>
Example for SOCKS 'associate' command</h1>
<p>The associate command tells the SOCKS proxy server to establish a UDP relay. The server binds to a new UDP port and communicates the newly opened port back to the origin client. From here, any SOCKS UDP frame packets sent to this special UDP port on the <a class="el" href="classProxy.html" title="Do nothing but invoke all child nodes Since the node is not actually &quot;no operation&quot;,...">Proxy</a> server will be forwarded to the desired destination, and any responses will be forwarded back to the origin client (you).</p>
<p>This can be used for things such as DNS queries, and other UDP communicates.</p>
<p><b>Connection Steps</b></p>
<ol type="1">
<li>Client -(associate)-&gt; <a class="el" href="classProxy.html" title="Do nothing but invoke all child nodes Since the node is not actually &quot;no operation&quot;,...">Proxy</a> (Tells the proxy to create a UDP relay and bind on a new port)</li>
<li>Client &lt;-(port)- <a class="el" href="classProxy.html" title="Do nothing but invoke all child nodes Since the node is not actually &quot;no operation&quot;,...">Proxy</a> (Tells the origin client which port it opened and is accepting UDP frame packets on)</li>
</ol>
<p>At this point the proxy is accepting UDP frames on the specified port.</p>
<ol type="1">
<li>Client &ndash;(udp frame) -&gt; <a class="el" href="classProxy.html" title="Do nothing but invoke all child nodes Since the node is not actually &quot;no operation&quot;,...">Proxy</a> -&gt; Destination (The origin client sends a UDP frame to the proxy on the UDP port, and the proxy then forwards it to the destination specified in the UDP frame.)</li>
<li>Client &lt;&ndash;(udp frame) &lt;&ndash; <a class="el" href="classProxy.html" title="Do nothing but invoke all child nodes Since the node is not actually &quot;no operation&quot;,...">Proxy</a> &lt;&ndash; Destination (The destination client responds to the udp packet sent in #3)</li>
</ol>
<h1><a class="anchor" id="autotoc_md10396"></a>
Usage</h1>
...
<p><b>Note:</b> UDP packets relayed through the proxy servers are packaged in a special Socks UDP frame format. SocksClient.createUDPFrame() and SocksClient.parseUDPFrame() create and parse these special UDP packets.</p>
<div class="fragment"><div class="line">import * as dgram from &#39;dgram&#39;;</div>
<div class="line">import { SocksClient, SocksClientOptions } from &#39;socks&#39;;</div>
<div class="line"> </div>
